...
svg.append("text").attr("x", 20).attr("y", 40).attr("font-size", 24)
          .text("D3 v4.x Modules"), d3.csv("./data/modules_3.csv", function(a, b) {
    if (a) throw a;
    var c = d3.stratify().id(function(a) {
        ...
        d = 40,
        e = d3.pack().size([width, height]).radius(function() {
            return d
        }).padding(3),
        f = d3.scaleOrdinal(d3.schemePastel1);
