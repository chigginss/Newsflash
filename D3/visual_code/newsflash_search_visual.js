...
function makeCircles(response) {
  ...
  let nodes = data.map((d) => { 
    // Outlet.query.filter(Outlet.outlet_popularity if Outlet.outlet_name == d.source.name).first()
  ...
  d = {
    ...
    // Outlet.query.filter(Outlet.outlet_bias if Outlet.outlet_name == d.source.name).first()
    bias: outlets.get(d.source.name).bias,
    // Outlet.query.filter(Outlet.outlet_bias_key if Outlet.outlet_name == d.source.name && (Outlet.query.filter(Outlet.outlet_bias if Outlet.outlet_name == d.source.name).first())).first()
